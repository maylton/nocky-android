...
/**
 * Converts the app's existing persistent queue/player state into the portable
 * Nocky Connect protocol shape.
 *
 * This intentionally does not bind to MusicService or Media3 controller objects.
 * It is safe to unit-test and can be used by future debug export, LAN handoff,
 * or QR/pairing flows without changing playback behaviour yet.
 */
...
fun MediaMetadata.toPortableQueueItem(
    ...
    return PortableQueueItem(
        ...
        local = if (source == NockyConnectSource.LOCAL) {
            LocalTrackIdentity(
                libraryId = "android-local-library",
            )
        } else {
            null
        },
