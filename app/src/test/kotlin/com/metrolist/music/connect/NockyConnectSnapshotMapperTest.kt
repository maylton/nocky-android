/*
 * Nocky Connect integration tests
...
class NockyConnectSnapshotMapperTest {
    ...
    fun codecRoundTripsSnapshotJson() {
        ...
        assertEquals("playing", payload.substringAfter("\"state\": \"").substringBefore("\""))
        assertEquals("youtube", payload.substringAfter("\"source\": \"").substringBefore("\""))
    }

    @Test
    fun doesNotExportPrivateOrAccountFields() {
        val song = firstSong().copy(
            libraryAddToken = "private-add-token",
            libraryRemoveToken = "private-remove-token",
            uploadEntityId = "private-upload-entity",
            liked = true,
        )
        val snapshot = NockyConnectSnapshotMapper.fromPersistedState(
            queue = PersistQueue(
                title = "Private source queue",
                items = listOf(song),
                mediaItemIndex = 0,
                position = 0L,
            ),
        ...
        assertFalse(payload.contains("private-add-token"))
        assertFalse(payload.contains("private-remove-token"))
        assertFalse(payload.contains("private-upload-entity"))
        assertFalse(payload.contains("libraryAddToken"))
        assertFalse(payload.contains("libraryRemoveToken"))
        assertFalse(payload.contains("uploadEntityId"))
        assertFalse(payload.contains("cookie", ignoreCase = true))
        assertFalse(payload.contains("authorization", ignoreCase = true))
    }
